...
async def process_pair(user, pair: dict, db, settings: dict):
    ...
    last = (await db.execute(
        select(AlertHistory)
        .where(AlertHistory.user_id == user.user_id, AlertHistory.token_address == pair["baseToken"]["address"])
    ...
    await db.commit()
    logger.info(f"🚨 ALERT sent to {user.user_id} | {pair.get('baseToken', {}).get('symbol')}")

def start_scanner():
    scheduler.add_job(scan_watchlists, "interval", seconds=config.SCAN_INTERVAL_SECONDS, id="scanner")
    scheduler.start()
    logger.success("📡 Live DEX Screener scanner started")
